...
def main():
    ...
    selected_features = preprocessor.named_steps[
        "feature_selector"
    ...
    variance_ratios = preprocessor.named_steps[
        "feature_selector"
    ...
    def try_feature_selectors(
        feature_selector: PCA | SelectKBest, log_y: bool = False
    ) -> pd.DataFrame:
        """
        Reduces Dimensionality of Feature Space with a given Feature Selector, fits all
        Models including Hyperparameter Tuning and returns a Pandas DataFrame with the
        Results. This function is used to concatenate the Results for all Feature
        Selectors to compare their effectiveness.
        """
    ...
    pd.concat(k_best_results).to_pickle("../../data/results/k_best_results.pkl")
    ...
    pd.concat(rfe_results).to_pickle("../../data/results/rfe_results.pkl")
    ...
    pd.concat(sfm_results).to_pickle("../../data/results/sfm_results.pkl")
    ...
    pd.concat(vt_results).to_pickle("../../data/results/vt_results.pkl")
    ...
    pd.concat(pca_results).to_pickle("../../data/results/pca_results.pkl")
