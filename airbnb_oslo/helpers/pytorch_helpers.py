...
def generate_train_val_data_split(
    ...
    """
    Splits the entire Dataset used for Model Training into a Training Set and Validation
    Set. The relative Sizes of each Output Component can be specified with a fractional
...
def generate_subsets(
    ...
    """
    Returns Training and Validation Sets of Reduced Size by randomly sampling
    observations from the original Training and Validation Sets. The Sizes can be
    specified with an Integer Input Value.

    This procedure is useful at early Stages of Model Construction for confirming a
    correct Model Setup as well as for Debugging on a CPU.
    """
...
def print_param_shapes(model: Any, col_widths: tuple[int, int] = (25, 8)):
    """
    Prints the Shape of the Weight Tensors between all Layers in a Neural Network as
    well as the total Number of Parameters trained during Model Fitting.
    """
...
def _print_shape(input: torch.Tensor, layer: Any | None = None, col_width: int = 25):
    """
...
def print_data_shapes(
    ...
    exclude: nn.Sequential | list = nn.Sequential,
):
    """
    Prints the Shape of the Output Tensor for each Layer in a Neural Network. This can
    be particularly helpful for specifiying the correct dimensions after flattening a
    four-dimensional Tensor in a Convolutional Network.
    """
...
class MLP(nn.Module):
    """
    Architecture of Fully-Connected Neural Network for all numeric / metric Input
    Features.
    """
...
class NeuralNetMetrics:
    """
    Tracks Performance Metrics for all Epochs during Model Training. This is
    particularly useful to plot Loss Curves of the Mean Squared Error, Mean Absolute
    Error and R^2 Value on Training and Validation Set after Training is completed.
    """
    ...
    def plot(self):
        ...
        ax1.set_yticklabels([f"{int(x):,}" for x in ticks_loc])
...
def init_weights(layer: nn.Module, mean: float = 0, std: float = 1):
    """
    Initializes Model Weights at the Start of Training. Avoids negative predicted Prices
    during the first Epochs to take the Logarithm when training with Log-Prices.
    """
...
def train_regression(
    ...
    Returns Mean Squared Error, Mean Absolute Error and R^2 Value for this Epoch on the
    Training Set.
    """
...
def validate_regression(
    ...
    """
    Evaluation Step on the Validation Set for a single Epoch. Returns Mean Squared
    Error, Mean Absolute Error and R^2 Value for this Epoch on the Validation Set.
    """
...
def print_epoch(
    ...
    """
    Prints Information about the Model Performance in Training and Validation Set of the
    current Epoch while the Model is trained.
    """
...
def print_best(
    ...
    """
    Prints the lowest Mean Absolute Error of the Training and Validation Set encountered
    during Model Training after Training is completed.
    """
...
def run_regression(
    ...
    save_path: bool = None,
    verbose: bool = False,
) -> tuple[NeuralNetMetrics, ResultContainer] | NeuralNetMetrics:
    """
    Trains a Neural Network Regression Model for a specified number of Epochs. During
    Training Performance Information is displayed. Optionally the State with the lowest
    Mean Absolute Error can be tracked and saved.

    Returns a NeuralNetMetrics Object for plotting Loss Curves and optionally a
    ResultContainer Object to collect the Results in a Pandas DataFrame for Comparison
    with the Classical Statistical Models.
    """
...
def show_images(
    ...
    """
    Displays Grid of Apartment Images with their true Prices and their predicted Prices
    from the Preatrained Convolutional Neural Network. The predicted Price is the mean
    of all predicted Prices for one specific Apartment.
    """
