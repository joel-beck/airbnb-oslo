...
\begin{frame}{Evaluation Metrics}
  \begin{itemize}
    \item Mean Squared Error for \emph{Training}, Mean Absolute Error and $R^2$ for \emph{Evaluation} \pause
    \item When using \textbf{log-price} for model fitting, MAE and $R^2$ are computed on the \textbf{original} price scale for better interpretability \pause
    \item Metrics highly depend on the exact evaluation procedure:
          %%%
          \begin{align*}
...
\begin{frame}{Main Findings}
  \begin{itemize}
    \item More features tend to work better with diminishing returns ($5$ vs. all $59$ features) \pause
    \item Price prediction task with small tabular data does not require overly complex models \\
          $\Rightarrow$ similar performance of linear and nonlinear classical models,  linear models generalize better to validation set \pause
...
\begin{frame}{Main Findings}
  \begin{itemize}
    \item Without outlier removal we can expect a MAE of $400$ NOK (40 Euros) and a $R^2$ value of around $0.4$. \pause
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item Large impact of outliers \pause
